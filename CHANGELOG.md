...
### Added
- **8 new signal-group YAMLs** under `roles/signals/`: `docker`, `python`, `message-queue`, `caching`, `mobile`, `ci-cd`, `crypto`, `llm`. Selector coverage was previously limited to `api / auth / frontend / sql / terraform` — domain artefacts in containerised, Python, queue-driven, mobile, CI/CD, cryptographic, or LLM contexts fell through to the bare role lens. Each new signal augments 2–4 existing roles with 7–8 checklist items in the same action-first / one-line-rationale style as the originals. No pipeline, schema, or contract change — selector loads `signals/*.yaml` directly at runtime, no index rebuild needed.
- **`scripts/validate-signals.py` + `make test-signals`** — schema-check for `roles/signals/*.yaml` (required keys, group-equals-stem, no duplicate group / id, regex matchers compile, plain matchers ≥2 chars, every `augments_roles` entry exists in `roles/index.json`). Wired into `make test` alongside `test-index` and `test-handoff` so a typo in a new signal fails CI rather than silently shipping a never-matching matcher. README under `roles/signals/` corrected — old hint pointed at `make build-index`, which doesn't touch signals.
...
- **Timing instrumentation in `_index.json`** — two new optional, additive fields written by sub-coordinators and main during a run:
  - `coordinator_spawns[]` — one entry per Phase A/B/C subagent spawn, with `phase`, `spawn_n`, `resume_token`, `started_at` (ISO-8601 UTC at entry), `emit_at` (at return), `action` (`complete` / `dispatch` / `error` / `aborted`), `step_emitted` (dispatch step or null). Sub-coordinators append on entry, close on emit. Crashed spawns leave `emit_at: null` — useful postmortem signal.
  - `dispatch[]` — superset of the prior `{role, model, step}` shape that step-7 instructions wrote. New optional fields: `subagent_type`, `started_at`, `completed_at`, `duration_ms`, `status` (`ok` / `skipped` / `error`), `reason`, `attempt` (1-based; retries append a new entry). Main writes per-request before/after each `Agent` call during a dispatch handoff.
  - Both fields are **optional and additive**: runs older than this instrumentation lacked them; readers must treat absence as `[]`. **No `phase-handoff.json` schema changes** — `_index.json` is workspace state, not part of the handoff contract. Backward-compat by construction.
  - Touched files: `meta-agents/sub-coordinator-phase-{a,b,c}.md` (new "Timing instrumentation" sections + entry/emit hooks), `SKILL.md` (new "Timing instrumentation" section + dispatch-execution hooks).
  - Why: optimization ideas like "Phase C single dispatch", "drift pre-check overlap with adversarial", "coordinator markdown trim" have plausible-sounding wins but unmeasured magnitude. With per-spawn and per-dispatch wall-clock numbers in `_index.json`, future PRs land or are rejected on data, not on intuition.
- **`docs/specs/2026-05-07-pyramidal-preflight-future-direction.md`** — design-level note recording the user-proposed "pyramidal preflight" idea (cascade panels from architecture down to implementation detail, possibly even style), why it's deferred (cost explosion past design layer; LLM panels are wrong tool below design; conflict with sibling skills), and the two forms worth revisiting later (`depth=detailed/cascade` parameter on /preflight; `preflight-cascade` sibling skill). Decision: defer until timing instrumentation produces real measurements. Out of scope permanently: auto-cascade to style / indentation / formatting (formatter/linter territory).
...
### Changed
- **Gate UX — `AskUserQuestion` picker.** The Phase A → main handoff now drives an interactive picker instead of a Markdown blob. `phase_a_output.gate` carries a new `gate_json_path` (abs path to the structured `$WORKSPACE/gate.json`) and the `questions_count` cap drops from 5 to 4 to fit `AskUserQuestion`'s 1–4 questions-per-call limit. `gate.json` questions gain a `header` field (≤12 chars, `user_language`) for the picker chip. Main-session flow: read `gate.json`, render `binary`/`choice` through one `AskUserQuestion` call (option `description` = `+ <pros>\n− <cons>`), follow up free-form on `open` questions and on options whose `pros` mentions `paste`/`probe output` (the deploy-state probe paste). `gate.md` is kept as fallback render for the schema-invalid / all-`open` / `render_too_long` cases; legacy `"1=a 2=b"` parsing stays in the fallback path. Material-change rebranch heuristics now run on free-form text only (`Other`+notes, `open` replies, fallback replies) — explicit signals: file-path tokens, `file:line` refs, the literal `actually`, four-digit line numbers, brief contradictions. See `docs/specs/2026-05-08-ask-user-question-gate.md`. Touched: `schemas/phase-handoff.json`, `meta-agents/sub-coordinator-phase-a.md`, `SKILL.md`.
